...
SP7 中有一批「平常看不到、踩到才知道」的預設限制：筆數、大小、逾時等。它們散落在不同原始碼、config、元件裡，出問題時常被當成「功能 bug」追查半天。這份文件把這類限制集中在一處，遇到類似症狀先掃這張表。

> 每次發現新的隱藏限制，就在這裡補一行；不要把同一段寫進每個元件 doc。
...
**踩雷症狀**：客戶按「匯出 Excel」後拿到的檔案筆數固定 ≤ 1000，但查詢清單明明更多。
...
**踩雷症狀**：複雜 SQL 或大批資料查詢拋 `timeout expired` 例外。
...
**踩雷症狀**：超大檔上傳時，IIS 回 **413 Payload Too Large**，此時連 ASP.NET 都還沒接到請求。
...
**踩雷症狀**：使用 AD 同步時，成員數超過 1000 的群組抓不全。
...
**踩雷症狀**：呼叫 GPT/Claude 長回應或 streaming 一開始就被中斷。10 分鐘通常夠用，除非跑 long-thinking 的 model。

### 7. FileProvider 上傳（無限）

**非踩雷**：SP7 已主動在該 endpoint 加 `[RequestFormLimits(MultipartBodyLengthLimit = long.MaxValue)]`，繞過 ASP.NET 預設的 128MB 表單限制。**上限實際是 IIS 層的 1GB**（見第 4 條）。

---

## 如何維護這份表

- **發現新地雷**：補一行到對照表，再視情況加一段細節說明。欄位：項目 / 預設值 / 調整方式 / 位置（含行號）。
