...
五個 DB 都含完整 9 個欄位，**無差異**。

#### 型別對照

| 欄位 | MSSQL | Oracle | MySQL | DB2 | Informix |
|------|-------|--------|-------|-----|----------|
| `IsRead` | `bit` | `CHAR(1)` | `bit` | `DECIMAL(1,0)` | `DECIMAL(1,0)` |

> ℹ️ Boolean 欄位跨 DB 型別不統一 —— 讀取端需處理 `true`/`'Y'`/`1` 多種表示。
...
其他 DB 的 CREATE TABLE 都已含 `ProjectID`，舊版升級時需手動補（SQL 範本同 FLOWINSTANCE 小節）。
