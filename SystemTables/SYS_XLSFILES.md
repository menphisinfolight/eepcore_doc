...
⚠️ **除 MSSQL 外全部 DB 新裝都缺 `HEADFORMAT`**（Excel 表頭功能失效）。
⚠️ **DB2 / Informix 額外缺 `KEYFIELDS` 與 `NAMEFIELDS`**（主鍵 / 名稱欄位設定）。
⚠️ DB2 / Informix `QUERYFORMAT` 上限 2000 字元。

👉 完整欄位存在度、升級 ALTER 支援矩陣、手動補欄位 SQL：**問題_SP7_跨資料庫欄位差異盤點**
