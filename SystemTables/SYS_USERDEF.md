...
⚠️ **日期欄位名跨 DB 不一致**：MSSQL / Oracle / MySQL 叫 `DEFDATE`，**DB2 / Informix 叫 `DATE`**（DB2 還用反引號包裹）。
⚠️ **DB2 新裝缺 `RFORMAT`**（報表格式化失效）。

兩個問題都像腳本撰寫誤植。

👉 完整欄位存在度、升級 ALTER 支援矩陣、手動補欄位 / 修欄位名 SQL：**問題_SP7_跨資料庫欄位差異盤點**
