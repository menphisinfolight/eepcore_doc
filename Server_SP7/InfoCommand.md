...
|----------|--------------|------------|
| `None` | — | 不過濾 |
| `User` | 使用者欄位 | `{secField} = {ClientInfo.User}` |
| `Group` | 群組欄位 | `{secField} IN ({ClientInfo.Groups})` |
| `Role` | 角色欄位 | `{secField} IN ({ClientInfo.Roles})` |
| `Org` | 組織角色欄位 | `{secField} IN ({ClientInfo.OrgRoles})` |
| `FlowHistory` | —（不需 secField） | `SUBSTR(FlowFlag,3,36) IN (SELECT InstanceID FROM FlowHistory … UNION SELECT InstanceID FROM FlowToDo …)` |

> Group/Role/Org 三種模式若 `ClientInfo` 對應欄位為空陣列，會補上 `1=0` 讓查詢回傳零筆（避免意外洩漏）。
...
設定排除名單，符合的使用者/群組不受安全過濾限制。以 `,` 或 `;` 分隔（原始碼 `SecExcept.Split(",;".ToCharArray())`）。
...
> ⚠️ **SP7 EEP Core 沒有「資料庫後綴自動路由」機制**。`ServerComponent.OnEvent` 只會把 JSON 中設定的單一方法名（例如 `menu_onBeforeExecuteSQL`）傳給 `Module.InvokeMethod`，不會自動尋找 `menu_onBeforeExecuteSQL_Oracle` 之類的變體（已全原始碼 grep `_Oracle` / `_Informix` / `_DB2` 皆無結果）。

要在同一個事件裡處理不同資料庫，請在方法內自行判斷：

```csharp
public string menu_onBeforeExecuteSQL(object sender, string sql, List<string> whereStrs)
{
    var db = (sender as InfoCommand).Module.DbHelper;
    // DatabaseHelper.GetDbType() / Connection.GetType().Name / 檢查欄位標記方式等
    if (db is InformixDatabaseHelper)
        return $"{sql} WHERE \"ITEMTYPE\" = {db.MarkValue(...)}";
    else
        return $"{sql} WHERE ITEMTYPE = {db.MarkValue(...)}";
}
```

或者在設計階段：針對不同資料庫準備不同 InfoCommand（`Database` 屬性指向對應連線），前端用 JS 切 `remoteName`。
...
### 範例 4：Informix 需要欄位名加雙引號（同一方法內判斷 DB 型別）

> 此範例原本用 `_Informix` 後綴命名，但 SP7 EEP Core 不會自動路由，**方法名只能是 JSON 設定的單一名稱**。實務上應把 DB 差異包進同一個方法：

```csharp
public string menu_onBeforeExecuteSQL(object sender, string sql, List<string> whereStrs)
{
    var command = sender as InfoCommand;
    var db = command.Module.DbHelper;
    var clientInfo = command.Module.ClientInfo;

    if (sql.IndexOf("ITEMTYPE") < 0)
    {
        // db.MarkColumn 會依資料庫產生正確的欄位括號（SQL Server: [col]、Oracle/Informix: "col"、MySQL: `col`）
        var col = db.MarkColumn("ITEMTYPE");
        return db.InsertWherePart(sql, $"{col}={MarkValue(clientInfo.Solution)}");
    }
    return sql;
}
```
...
{
    var clientInfo = (sender as InfoCommand).Module.ClientInfo;
    // SP7 ClientInfo 沒有 Variables 屬性；使用者自訂變數存放於 UserParameters Dictionary，
    // 透過 SetValue(name, value) 寫入、GetValue(name) 讀取（回傳 object，需 ToString）
    var filter = clientInfo.GetValue("myFilter")?.ToString();
...
> 注意：全域變數 `$.setVariableValue` 對應後端 `ClientInfo.UserParameters`（`Dictionary<string, string>`），**只能存字串**（[#481846](https://www.infolight.com/cloud_andyhome2_bootstrap/DISDT?type=010&id=481846)）。
