...
> 原始碼位置：`bootstrap.infolight.js` 第 9065–9438 行
> jQuery 外掛名稱：`$.fn.gauge`

### 初始化流程

1. 解析 `data-options`，呼叫 `load` 載入資料。

### 資料載入（load）

- **遠端模式**（`remoteName` 有值）：透過 `$.loadData()` 查詢（傳入 `total:false`、`whereStr`、`whereItems`），回傳後呼叫 `loadData`。
- **無資料來源**：直接以 `null` 呼叫 `loadData`，使用 `currentValue` 靜態值。

### 數值解析邏輯（loadData）

- **infocard 模式**（`infocard == true`）：從資料列的 JSON 字串中解析 `Field`、`MinValue`、`MaxValue` 欄位值。若 `style == 'zones'`，同時解析 `interval1`、`interval2`，自動建立三段色帶（綠→橙→紅）。
- **一般模式**：分別從 `MinField`、`MaxField`、`Field` 取值。
- 所有 NaN 值皆替換為 0。

### Radius 計算

根據 `Angle` 值動態調整 `Radius`：
- Angle >= 9：`Radius / 10`（donut+infocard）或 `Radius / (18 + (10 - Angle) * 2)`
- Angle == 8：`Radius / (13 + 4)`
- Angle 5~7：`Radius / (10 + (10 - Angle) * 2)`
- 其他：`Radius / 20`
...
| 樣式 | 說明 |
|------|------|
| **active** | 基本儀表，具指針（`pointer.length > 0`），使用漸層色（`colorStart`/`colorStop`）。標籤絕對定位在 `LablePosition` 處。 |
| **donut** | 環形儀表，指針長度與寬度皆為 0（無指針）。標籤位置根據 `Angle` 自動計算（Angle 10→42%、9→43%、8→47%、7→52%、6→65%、<=5→78%）。 |
| **zones** | 區間儀表，具指針。將 `StaticZones` 的百分比範圍換算為實際 min/max 值後傳入。若 `ViewStaticLabels` 為 true，在區間邊界顯示刻度數字。若數值超出 min/max 範圍，自動擴展區間。 |

### Canvas 渲染

所有樣式皆透過第三方 `Gauge` 物件（gauge.js）繪製於 `<canvas>` 上：
- 建立 `new Gauge(canvasElement, height, width)`。
- 呼叫 `setOptions(opts)` 設定外觀。
- 若 `ViewLabels` 為 true，呼叫 `setTextField()` 綁定文字顯示元素。
- 最終呼叫 `gauge.set(value)` 設定指針值。
...
| `init` | 初始化並載入資料 |
| `load` | 依資料來源載入 |
| `loadData` | 解析數值、計算區間、建立 Gauge 繪製 |
| `setWhere` | 預留方法（目前為空實作） |
